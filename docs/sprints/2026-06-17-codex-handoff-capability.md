...
CURRENT STATE: Sprints 1–2 shipped (charts, offline icons, security/privacy
hardening, CI gate, ai-lab CLI). Gate is green (~190 tests), CI passes on GitHub.
...
L0 docs/cli: (a) Fix default-model onboarding in src/local_ai_lab/cli/doctor.py
...
L1 cli: ai-lab hardware snapshot (per Sprint 3 Loop 1) — JSON, --out, NO private
   paths/usernames/env/secrets, no model calls. Smoke: ai-lab hardware snapshot.
L2 cli/bench: ai-lab bench matrix (per Sprint 3 Loop 2) — reads candidates.csv,
   Markdown default + --json, marks blocked rows, runs NO models. Smoke:
   ai-lab bench matrix --limit 5.
L3 bench/cli ⭐ CLOSE THE BENCHMARK EXECUTION LOOP (the keystone, approval-gated):
...
L4 dashboard: Capability view (per Sprint 3 Loop 3) PLUS surface the real perf
   series L3 produces via charts.py (tokens/sec, latency) + hardware context +
   readiness/artifact counts. Read-only, no network, no private data. Smoke:
   model_dashboard_smoke.py.
...
L6 docs: Portfolio evidence pack (per Sprint 3 Loop 4) — truthful + locally
   verifiable; reflect the real executed-benchmark capability from L3.
...
- **L3 is the one that touches a real model.** When Codex reaches it, you'll need
  an installed local model and to approve the exact model id / runtime / run id.
  L0 fixes the default-model onboarding first, so `doctor` will guide you to an
  installed model before then.
